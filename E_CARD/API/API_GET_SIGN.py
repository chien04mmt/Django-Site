...
def Get_Signer_Info(userid):
    ...
    if len(SelectSQL(strquery))>0:return SelectSQL(strquery)[0]
    else: return ''
...
def List_SignInfo(docno):
    ...
    strq='''
                 SELECT  s.[Order_No],s.[Is_Check],s.Sign_Type,s.[Signer],s.[Status],u.[UserName],s.[Sign_Detail],s.[Sign_IP],s.[Sign_Time],s.[Sign_FileNameOld]
                    FROM 
                    ( 
                    SELECT  *
       ...
       '''
    sql=SelectSQL(strq)# Lấy danh sách các chữ ký
    return sql
...
def Get_waiting_ID(docno):
    ...
    if len(SelectSQL(strq))>0: 
        # print(strq)
        return SelectSQL(strq)[0]
    else: return ''
...
def GET_Status_DOC(docno):
    ...
    if len(SelectSQL(strsearch))>0: return SelectSQL(strsearch)[0]['Process']
    else: return ''
...
def Get_Applicant(docno):
    ...
    if len(SelectSQL(strq))>0 : return SelectSQL(strq)[0]
    else: return ''
...
def SET_STATTUS_DOC(docno,Sign_Type,Next_Approver,Next_Name,Process):
    str_updateH=" UPDATE [File_Signer_Apply_H] SET [Next_Approver] ='"+str(Next_Approver)+"',"+'''
            '''+ "[Process]='"+str(Process)+"',[Sign_Type] ='"+str(Sign_Type)+"',"+'''
            '''+ "[Next_Name]='"+str(Next_Name)+"' WHERE [Apply_No]='"+str(docno)+"'"
...
def GET_DOCNO_INFO(docno):
    ...
    sql1=SelectSQL(strqueryDoc)# Lấy thông tin ID đầu đơn
...
def GET_FLOWNAME_DETAIL(docno):
    ...
    if len(sql1)>0 : 
        ...
        flowdetail=SelectSQL(str_docname)
...
def GET_LIST_ATTACHMENT(docno):
    sql=SelectSQL("SELECT * FROM [File_Signer_Apply_F] WHERE Apply_No"+"='"+str(docno)+"'")
...
def INSERT_ATTACH(docno,File_Name):
    str_is1="INSERT INTO File_Signer_Apply_F([Apply_No],[File_Name]) VALUES('"+str(docno)+"','"+str(File_Name)+"')"
...
def CREATE_DOC_H(docno,Order_No,Flow_Name_Detail,Project_Name,Description,Apply_File,Site,bu,Level_Grade,Apply_Person,Apply_EmpNo,Apply_Tel,Apply_Notus,Process,Next_Approver,Next_Name,Remark,MailCC,MailCC_End):
    ...
    strquery2='''
            ...
            VALUES ('''+ "'"+ str(docno) +"','"+ str(Order_No) +"',N'"+ str(Flow_Name_Detail) +"',N'"+ str(Project_Name)  +"',N'"+ str(Description) +"',N'"+ str(Apply_File)+"',N'"+ str(Site)+"',N'"+ str(bu)+"','"+str(Level_Grade) +"',N'"+str(Apply_Person )+'''
            ','''+ "'"+str(Apply_EmpNo)+"','"+ str(Apply_Tel)+"',N'"+str(Apply_Notus)+"', SYSDATETIME(),'"+str(Process)+"',N'"+str(Next_Approver)+"',N'"+Next_Name+"','N',N'"+str(Remark) +"',N'"+MailCC +"',N'"+ MailCC_End+"')"# Câu lệnh chèn  thông tin đơn vào bảng [File_Signer_Apply_H]    
    if QuerySQL2(strquery2): return True
    else: return False
